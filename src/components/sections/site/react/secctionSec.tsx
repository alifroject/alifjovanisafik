...
export const CertificateFrame: React.FC = () => {
    const overlayText = `React is a JavaScript library developed by Meta (formerly Facebook) for building user interfaces, especially single-page applications (SPAs), using reusable components and a virtual DOM for efficient rendering. Next.js is a powerful React framework developed by Vercel that extends React's capabilities by adding features like server-side rendering (SSR), static site generation (SSG), API routes, routing out of the box, and full-stack capabilities. While React handles the UI logic and rendering, Next.js provides the structure and optimizations needed for building production-ready web applications with improved performance, SEO, and scalability.`;
